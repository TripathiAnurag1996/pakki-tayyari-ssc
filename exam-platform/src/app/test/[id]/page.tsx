...
export default async function TestPage({
  ...
  if (session.chapter && session.chapter !== 'all') query = query.eq('chapter', session.chapter)
  ...
  if (questionsError || !questions || questions.length === 0) {
    ...
    return <TestInterface sessionId={session.id} initialQuestions={(randomQs || []) as Question[]} />
  }

  return <TestInterface sessionId={session.id} initialQuestions={questions as Question[]} />
}
